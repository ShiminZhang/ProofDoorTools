...
def separate_cnf_files(K):
    """Separate combined CNF files into appropriate directories based on their basenames."""
    cnf_dir = f"./ProofDoorBenchmark/cnfs/{K}"
    global linear_instances, polynomial_instances, exponential_instances
    # linear_instances = linear_instances[0:15]
    # polynomial_instances = polynomial_instances[0:15]
    # exponential_instances = exponential_instances[0:15]
    
    for filename in os.listdir(cnf_dir):
        if filename.endswith('.cnf'):
            basename = get_basename(filename)
            if basename not in linear_instances and basename not in polynomial_instances and basename not in exponential_instances:
                os.remove(os.path.join(cnf_dir, filename))
            

def separate_files():
    global linear_instances, polynomial_instances, exponential_instances
    """Separate combined CNF files into appropriate directories based on their basenames."""
    aig_dir = "./ProofDoorBenchmark/aigs/"
    
    # Check if the combined directory exists
    if not os.path.exists(aig_dir):
        print(f"Error: Directory {aig_dir} does not exist.")
        return
    
    # Create sets of instance names for faster lookup
    linear_instances_set = set(linear_instances)
    polynomial_instances_set = set(polynomial_instances)
    exponential_instances_set = set(exponential_instances)
    
    # Process each file in the combined directory
    for filename in os.listdir(aig_dir):
        if filename.endswith('.aig'):
            basename = get_basename(filename)
            source_path = os.path.join(aig_dir, filename)
            
            if basename not in linear_instances_set and basename not in polynomial_instances_set and basename not in exponential_instances_set  :

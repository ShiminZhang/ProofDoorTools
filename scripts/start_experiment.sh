...
if [ -n "$1" ] && [ -z "$2" ]; then
    ...
    for (( i=0; i<linear_instances_l; i++ )); do
        ...
        # cp $cnf_path ./test/interpolant_sanity/cnfs/$name.$k_value.cnf
        # mkdir -p ./test/interpolant_sanity/smts/
        ...
        for (( j=0; j<k_value; j++ )); do
            ...
            if [ -f "$smt_path" ]; then
                # cp $smt_path ./test/interpolant_sanity/smts/$name.$k_value.$j.smt2
    ...
    # for file in ./test/interpolant_sanity/smts/*.smt2; do
        ...
        if [ -f "$file" ]; then
            ...
            if [ ! -f "./ProofDoorBenchmark/interpolants/$1/$filename.interpolant" ]; then
                ...
                jobid=$(sbatch --priority 0 -o ./Outputs/interpolant_${filename}.out ./scripts/check_interpolant.sh $filename ./ProofDoorBenchmark/smts/$1/ ./test/interpolant_sanity/smts/ | awk '{print $4}')  

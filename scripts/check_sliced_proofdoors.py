...
def main():
    ...
    if not use_cache:
        ...
        with open(f"slicedexp_{solver}.json", "r") as f:
            original_combined_map = json.load(f)
    ...
    modified_original_combined_map = {}
        ...
        if type(times) == list:
            ...
            for i in range(len(times)):
                if times[i] == -1:
                    ...
                    new_times[str(i)] = times[i]
            times = new_times
        modified_original_combined_map[basename] = times
    original_combined_map = modified_original_combined_map
    
    for basename, times in original_combined_map.items():
        print(f"\nAnalyzing {basename}:")
        
        # Prepare data for regression
        indices = sorted( [int(k) for k in times.keys()] )
        values = [times[str(idx)] for idx in indices]

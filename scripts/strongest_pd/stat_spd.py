...
INTERPOLANT_DIR_TEMPLATE = "./ProofDoorBenchmark/interpolants_def5/{K}"
...
def _collect_plot_series(
    ...
    if not series_by_name:
        raise RuntimeError("no successful SPD rows available for plotting")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Collect strongest-PD computation status and interpolant clause counts "
            "into a CSV file."
        )
    )
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="CSV output path (default: ./spd_stats_k_<K>.csv)",
    )
    parser.add_argument(
        "--input_csv",
        default=None,
    ...
    if args.output is not None:
        ...
        output_path = f"./spd_stats_k_{args.K}.csv"
    else:
        output_path = f"./spd_stats_k_{K_values[0]}_to_{K_values[-1]}.csv"

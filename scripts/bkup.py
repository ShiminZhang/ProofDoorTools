...
from utils.paths import get_interpolant_dir, get_interpolant_cnf_dir
...
_CNF_CACHE = {}
_MERGE_MAX_WORKERS = 1
_MERGE_PARALLEL_THRESHOLD = 64
    ...
    if not simplify:
        ...
        # Disjunction: apply distributive law incrementally to avoid materializing the full Cartesian product
            ...
            for sub_cnf in sub_cnf_list:
                ...
                new_clauses = []
                seen = set()
                workload = len(combined) * len(sub_clause_sets)
                if _MERGE_MAX_WORKERS > 1 and workload >= _MERGE_PARALLEL_THRESHOLD:
                    with ThreadPoolExecutor(max_workers=_MERGE_MAX_WORKERS) as executor:
                        for partial in executor.map(
                            _merge_clause_with_subs, ((base_clause, sub_clause_sets) for base_clause in combined)
                        ):
                            for merged_literals in partial:
...
def _merge_clause_with_subs(args):
    base_clause, sub_clause_sets = args
    local_seen = set()
    local_result = []
    for clause_set in sub_clause_sets:
        merged_literals = base_clause | clause_set
        if _is_tautological_clause(merged_literals):
            continue
        if merged_literals in local_seen:
            continue
        local_seen.add(merged_literals)
        local_result.append(merged_literals)
...
def InterpolantToCNF(instance, K, index, simplify=False, workers=1):
    global _MERGE_MAX_WORKERS
    _MERGE_MAX_WORKERS = max(1, workers)
    _CNF_CACHE.clear()
...
if __name__ == "__main__":
    ...
    if args.main:
        ...
        log_dir = f"Experiments/SMTTranslationToCNFExperiment/logs/{args.check_result}"
        ...
        dict_results = {log_file: result for log_file, result in sorted_results}

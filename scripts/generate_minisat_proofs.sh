...
for cnf_file in "$directory"/*.cnf; do
    if [ -f "$cnf_file" ]; then
        ...
        jobid=$(sbatch --priority 0 -o ./Outputs/output_%A_%a.out --mem=4g --time=1:00:00 --wrap="./solvers/minisat_pf $cnf_file | grep 'PDLOG Learnt clause:' | sed 's/PDLOG Learnt clause: //' > $log_file")

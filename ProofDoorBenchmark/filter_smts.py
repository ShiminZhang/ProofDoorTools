...
def separate_files():
    ...
    if not os.path.exists(smt_dir):
        print(f"Error: Directory {smt_dir} does not exist.")
        return
    
    # Create sets of instance names for faster lookup
    linear_instances_set = set(linear_instances[0:15])
    polynomial_instances_set = set(polynomial_instances[0:15])
    exponential_instances_set = set(exponential_instances[0:15])
    
    # Process each file in the combined directory
    for filename in os.listdir(smt_dir):
        if filename.endswith('.smt2'):
            basename = get_basename(filename)
            source_path = os.path.join(smt_dir, filename)

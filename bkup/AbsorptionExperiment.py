...
def check_single_clause_absorption_worker(args):
    ...
    instance, K, proof_index, interpolation_index, interpolant_clause_index, interpolant_clause, use_minisat_proof, use_glucose_proof, include_formula_in_checking, permute, permute_index = args
    ...
    return check_formula_absorp_clause(
        proof_clauses,
        interpolant_clause,
        f"{instance}.{K}.interpolation_{interpolation_index}.interpolant_{interpolant_clause_index}.proof_{proof_index}{permute_flag_to_suffix(permute, permute_index)}.{solver_suffix}{include_formula_in_checking_flag_to_suffix(include_formula_in_checking)}.check_absorb.json",
        K,
    )
...
def get_smtcnf_path(instance: str, K: int, index: int, reverse: bool = False, permute: str = None, permute_index: int = 0) -> str:
    suffix = ".reverse.smtcnf" if reverse else ".smtcnf"
    perm_suffix = permute_flag_to_suffix(permute, permute_index)
    return f"{get_interpolant_cnf_dir(K,1)}/{instance}.{K}.{index}{perm_suffix}{suffix}"

def get_absorption_result_path(instance: str, K: int, interpolation_index: int, solver_suffix: str, include_formula_suffix: str, reverse: bool = False, base_dir: str = None, permute: str = None, permute_index: int = 0) -> str:
    reverse_suffix = ".reverse" if reverse else ""
    base = base_dir or get_absorption_experiments_dir(K)
    perm_suffix = permute_flag_to_suffix(permute, permute_index)
    return f"{base}/{instance}.i_{interpolation_index}{reverse_suffix}{perm_suffix}.{solver_suffix}.{include_formula_suffix}check_absorb.json"

class AbsorptionExperimentConfig(ExperimentConfig):
    def __init__(self, name, data_dir, result_dir, log_dir, K, category, force_instance=None, index = None, use_minisat_proof=False, use_glucose_proof=False, include_formula_in_checking=False, use_pbh_proof=False, reverse: bool = False, permute: str = None, permute_index: int = 0):
